...
public class GetBalanceQueryHandler : IRequestHandler<GetBalanceQuery, Result<List<UserBalanceDto>>>
{
    ...
    public async Task<Result<List<UserBalanceDto>>> Handle(GetBalanceQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            // БЕЗОПАСНОЕ СУММИРОВАНИЕ: кастуем к (decimal?)
            var owedToMe = await _context.ExpenseSplits.Where(s => s.Expense!.ApartmentId == request.ApartmentId && s.Expense.PayerId == request.CurrentUserId && s.UserId == roomie.UserId).SumAsync(s => (decimal?)s.Amount, cancellationToken) ?? 0m;
            var iOwe = await _context.ExpenseSplits.Where(s => s.Expense!.ApartmentId == request.ApartmentId && s.Expense.PayerId == roomie.UserId && s.UserId == request.CurrentUserId).SumAsync(s => (decimal?)s.Amount, cancellationToken) ?? 0m;
            var receivedFromRoomie = await _context.Settlements.Where(s => s.ApartmentId == request.ApartmentId && s.SenderId == roomie.UserId && s.ReceiverId == request.CurrentUserId).SumAsync(s => (decimal?)s.Amount, cancellationToken) ?? 0m;
            var sentToRoomie = await _context.Settlements.Where(s => s.ApartmentId == request.ApartmentId && s.SenderId == request.CurrentUserId && s.ReceiverId == roomie.UserId).SumAsync(s => (decimal?)s.Amount, cancellationToken) ?? 0m;
